...
The workspace `CLAUDE.md` has an `## Architecture` block: the user's name and pronouns, the architecture directory, the canonical document, the plan and review directories, the publisher (the pages dir and its URL), the session tools and the coordinator and implementer session names, the diagram renderer, the human-only actions. Every path, name, and tool you use comes from it.
